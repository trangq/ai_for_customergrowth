...
**Solution:** Models must be in `models/` folder. Run `project.ipynb` to generate them.

### ❌ Error: "Customer not found"
**Solution:** Check customer_id format - should be like "C00000" (C + 5 digits)

### ❌ API slow on first request
**Normal!** First request loads all models into memory (~2-5 seconds). Subsequent requests are instant.
...
### To retrain models:
1. Open `project.ipynb` in VS Code / Jupyter
2. Run all cells
3. Models auto-save to `models/` folder
4. Restart API server to load new models

### To modify API endpoints:
1. Edit `app/main.py`
2. API auto-reloads (due to `--reload` flag)
3. Check `http://localhost:8000/docs` for updated schema

### To add new features:
1. Create new scoring module in `app/scoring/`
2. Import in `app/main.py`
3. Add endpoint with FastAPI decorator

---

## Performance Notes

- **Single customer score:** ~100-200ms
- **Top 100 retention ranking:** ~10-20 seconds
- **Memory usage:** ~500MB (all models loaded)
- **Concurrent requests:** Supported (uvicorn handles threading)
